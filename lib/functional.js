...
async function runFunctional(url, opts = {}) {
  ...
  try {
    ...
    // Must match the inventory selector used to build `btns` above, or buttons.total counts
    // controls the click loop never looks at. It counted a.btn / a[class*=cta] CTAs and then
    // clicked none of them, reporting '4 buttons, 0 clicked' with no explanation at all.
    const BTN_SEL = 'button, input[type=button], input[type=submit], [role=button], a.btn, a[class*="buy"], a[class*="cta"]';
    ...
    // A page that advertises a price but exposes no reachable checkout has NOT had its
    // payment path tested. Reporting PASS/100 here is the failure that matters most: it
    // reads as 'payments work' when nothing was tried. The Walker Works apps sell from
    // /api/checkout behind sign-up, so a public scan can never reach it - say so, do not
    // quietly pass.
    // $0 is not a price you can pay. Only a NON-ZERO amount means there is something to
    // buy here - InvoiceMate's page shows only a free tier and was wrongly warned about.
    const amounts = ((inv.bodyText || '').match(/(?:US\$|A\$|\$)\s?[0-9][0-9,]*(?:\.[0-9]{2})?/g) || []);
    const payable = amounts.filter(a => parseFloat(a.replace(/[^0-9.]/g, '')) > 0);
    if (report.links.checkout.length === 0 && payable.length) {
      report.issues.push('This page shows a price but no checkout was reachable from it, so the payment path was NOT tested. If checkout sits behind sign-up or login, a public scan cannot reach it.');
    }
