...
async function runFunctional(url, opts = {}) {
  ...
  const report = {
    url, loaded: false, title: '', httpNote: '',
  ...
  try {
    ...
    if (flavour === 'puppeteer') await page.setViewport({ width: 1200, height: 900 });
    ...
    for (const l of inv.linkList) {
      if (report.links.followed.length >= maxLinks && !CHECKOUT_HOST.test(l.href)) continue;
      ...
      try {
        ...
        if (isCheckout) {
          ...
          const cd = await p2.evaluate(() => {
            ...
            const h = document.querySelector('h1, [class*="ProductSummary-name"], [data-testid*="product"]');
