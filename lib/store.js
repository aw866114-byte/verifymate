...
function db() {
  ...
  try {
    ...
    console.error('store: Firestore init failed:', String(e));
    _db = null;
  }
  return _db;
}
...
const NOT_WRITABLE = {
  ok: false,
  error: 'State store not configured — FIREBASE_SERVICE_ACCOUNT_KEY is missing on this Vercel project. Copy it from the walker-works project (Vercel → walker-works → Settings → Environment Variables), add it to verifymate, redeploy. Until then nothing can be remembered, so this write is refused rather than silently dropped.',
};

const COLLECTIONS = [
  'rules',      // AJ's own standing rules, added by him from /remember — merged over the seed rules
...
async function fullState() {
  ...
  const overlay = {};
  if (live) {
    const names = ['rules', 'facts', 'errata', 'queue', 'followups', 'clocks', 'inventory', 'coverage', 'incidents'];
    await Promise.all(names.map(async (n) => { overlay[n] = await readAll(n, 5000); }));
  }
  ...
  return {
    ...
    rules: mergeById(SEED.rules, (overlay.rules || []).filter((r) => !r.retired)),
...
module.exports = { db, col, writable, readAll, put, add, get, fullState, SEED, NOT_WRITABLE };
