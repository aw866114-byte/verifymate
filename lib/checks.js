...
const RUNNERS = {
  ...
  // Reads one NUMBER out of a JSON endpoint and holds it to a floor and/or ceiling.
  // The bearer token is NAMED here, never stored here - the value lives only in the env.
  // UNCONFIGURED IS A FAIL, LOUDLY. A check that could not read is not a check that passed.
  async json_metric(c) {
    const token = c.tokenEnv ? process.env[c.tokenEnv] : '';
    if (c.tokenEnv && !token) return { ok: false, value: 'unconfigured', evidence: `${c.tokenEnv} is not set on this project, so ${c.url} was never read. Paste it in Vercel and redeploy. Refusing to report a number it did not fetch.` };
    const ctl = new AbortController();
    const timer = setTimeout(() => ctl.abort(), 15000);
    let status = 0;
    let body = null;
    try { const r = await fetch(c.url, { signal: ctl.signal, headers: token ? { 'User-Agent': UA, Authorization: `Bearer ${token}` } : { 'User-Agent': UA } }); status = r.status; body = await r.json().catch(() => null); } finally { clearTimeout(timer); }
    if (status !== 200 || !body) return { ok: false, value: String(status), evidence: `GET ${c.url} → ${status}${body ? '' : ' (no JSON body)'}` };
    const raw = String(c.path).split('.').reduce((o, k) => (o == null ? o : o[k]), body);
    const n = Number(raw);
    if (!Number.isFinite(n)) return { ok: false, value: String(raw), evidence: `${c.url} → "${c.path}" is not a number (got ${JSON.stringify(raw)})` };
    const bounds = [c.min == null ? null : `min ${c.min}`, c.max == null ? null : `max ${c.max}`].filter(Boolean).join(', ') || 'no bound';
    const ok = (c.min == null || n >= c.min) && (c.max == null || n <= c.max);
    return { ok, value: String(n), evidence: `${c.url} → ${c.path} = ${n} (${bounds})` };
  },
};
